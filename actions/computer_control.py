...
def _safe_screenshot_path(requested: str | None) -> Path:
    ...
    fallback = temporary_output(prefix="captura-pantalla")
    if not requested:
        return fallback
    try:
...
def _scale_image_point(
    image_x: int,
    image_y: int,
    image_size: tuple[int, int],
    screen_size: tuple[int, int],
) -> tuple[int, int] | None:
    """Map physical screenshot pixels to logical mouse coordinates."""
    ...
    if not (0 <= image_x < image_w and 0 <= image_y < image_h):
        return None
    return (
        round(image_x * screen_w / image_w),
        round(image_y * screen_h / image_h),
    )
...
def _screen_find(description: str) -> tuple[int, int] | None:
    ...
    try:
        ...
        prompt = (
            f"This screenshot is exactly {image_w}×{image_h} pixels. "
        ...
        if match:
            image_x, image_y = int(match.group(1)), int(match.group(2))
            # Screenshots may use physical pixels while PyAutoGUI clicks in
            # logical coordinates when Windows display scaling is enabled.
            return _scale_image_point(
                image_x,
                image_y,
                (image_w, image_h),
                (screen_w, screen_h),
...
def computer_control(
    ...
    try:
        ...
        if action in ("screen_click", "screen_move", "screen_double_click", "screen_right_click"):
            desc   = params.get("description", "")
            coords = _screen_find(desc)
            if coords:
                _move(coords[0], coords[1], duration=0.35)
                if action == "screen_move":
                    return f"Moved to '{desc}' at {coords}"
                time.sleep(0.15)
                button = "right" if action == "screen_right_click" else "left"
                clicks = 2 if action == "screen_double_click" else 1
                _click(x=coords[0], y=coords[1], button=button, clicks=clicks)
                return f"{action} on '{desc}' at {coords}"

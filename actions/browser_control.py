...
def _open_in_system_browser(url: str) -> str:
    """
    Opens a URL with the operating system's normal/default browser.
    This reuses the user's real browser profile and logged-in sessions.
    """
    ...
    try:
        ...
        return f"Could not open: {url}. Error: {e}"
...
class _BrowserSession:
    ...
    async def screenshot(self, path: str = None) -> str:
        ...
        try:
            ...
            save_path = path or str(temporary_output(prefix="captura-navegador"))
...
def browser_control(
    ...
    result  = "Unknown action."
        # Simple URL opening should use the user's normal browser.
    # Avoid Playwright creating a separate browser window/profile.
    if action in ("go_to", "new_tab"):
        url = params.get("url", "").strip()

        if not url:
            result = "Please specify a URL."
        else:
            result = _open_in_system_browser(url)
    ...
    if action == "search":
        ...
        if not query:
            result = "Please specify a search query."
        else:
            ...
            result = _open_in_system_browser(base + quote_plus(query))

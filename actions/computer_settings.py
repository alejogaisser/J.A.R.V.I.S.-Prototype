...
def _get_windows_volume_interface():
    """Devuelve la interfaz de volumen principal de Windows."""
...
def _get_volume_percent() -> int:
    """Obtiene el volumen principal actual como porcentaje."""
    if _OS == "Windows":
...
def volume_mute():
    if _OS == "Windows":
        ...
        volume.SetMute(not currently_muted, None)
...
def volume_set(percent: int) -> None:
    if _OS != "Windows":
        raise RuntimeError("Volume control is only implemented for Windows.")
...
def _get_brightness_values() -> list[int]:
    """
    Devuelve el brillo de las pantallas detectadas.

    Normalmente devuelve algo como [50].
    Si hay varios monitores, puede devolver [50, 70].
    """
    import screen_brightness_control as sbc

    values = sbc.get_brightness(method="wmi")
...
def _get_brightness_percent() -> int:
    """
    Devuelve el brillo promedio de las pantallas detectadas.
    """
    values = _get_brightness_values()

    if not values:
        raise RuntimeError("No se pudo obtener el brillo de la pantalla.")
...
def brightness_set(value: int) -> None:
    """
    Establece el brillo entre 0 y 100.
    """
    ...
    if _OS == "Windows":
        ...
        # Fija el brillo en todas las pantallas compatibles.
        sbc.set_brightness(value, method="wmi")
        return

    if _OS == "Darwin":
        # macOS no está soportado directamente por
        # screen-brightness-control.
        current = _get_brightness_percent()

        if value > current:
            steps = max(1, round((value - current) / 10))
            ...
            steps = max(1, round((current - value) / 10))
...
def brightness_up() -> None:
    """
    Sube el brillo 10 puntos.
    """
    if _OS == "Windows":
        current = _get_brightness_percent()
        brightness_set(current + 10)
        return
...
- Return ONLY the JSON, no explanation, no markdown."""
    ...
    try:
        resp = _client.models.generate_content(model="gemini-3.5-flash", contents=prompt)
...
def computer_settings(
    ...
    # Normaliza nombres alternativos que puede generar el modelo.
    ACTION_ALIASES = {
    "set_brightness": "brightness_set",
    "change_brightness": "brightness_set",
    "brightness": "brightness_set",
    "brightness_to": "brightness_set",
    "set_screen_brightness": "brightness_set",
    "screen_brightness": "brightness_set",

    "set_volume": "volume_set",
    "change_volume": "volume_set",
    "volume": "volume_set",
    "volume_to": "volume_set",
}

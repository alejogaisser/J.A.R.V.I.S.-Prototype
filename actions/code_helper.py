...
def _take_screenshot() -> Path | None:
    try:
        ...
        screenshot_path = temporary_output(prefix="captura-debug")
...
def _generate_routine(description: str, language: str) -> str:
    lang = language or "python"
    response = _get_gemini().generate_content(
        f"Write clean, working {lang} code for this reusable routine: {description}. "
...
def _write_action(description, language, output_path, player, routine_name="") -> str:
    if not description:
    ...
    try:
        if routine_name:
            code = _generate_routine(description, language)
            register_script(routine_name, code, description, language)
            return f"Routine '{routine_name}' saved inside JARVIS memory.\n\nPreview:\n{_preview(code)}"
...
def code_helper(
    ...
    parameters:
        ...
        routine_name: Spoken name used to remember a reusable script
    """
    p           = parameters or {}
    ...
    routine_name = p.get("routine_name", "").strip()
    ...
    if action == "write":
        return _write_action(description, language, output_path, player, routine_name)
        ...
        if routine_name:
            return run_script(routine_name, timeout)

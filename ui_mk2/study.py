...
.step{display:grid;grid-template-columns:28px 1fr;gap:8px;margin:7px 0;padding:8px;border:1px solid #123947;background:rgba(3,23,31,.7)}.num{color:#4edfff}.note{color:#8dafb9;font-size:10px;margin:7px 0}.note:before{content:'◈ ';color:#4edfff}.source{color:#ffd36a;font-size:9px;margin-right:10px}
.md-p,.md-list,.md-quote,.md-code,.md-heading{margin:.35em 0}.md-list{padding-left:1.6em}.md-list li{margin:.28em 0}.md-quote{padding-left:9px;border-left:2px solid #2a7990;color:#9bc8d2}.md-code{padding:7px;background:#01070b;border:1px solid #123947;white-space:pre-wrap}.md-inline-code{padding:1px 4px;background:#071a22;color:#ffd36a}.md-heading{color:#b9f8ff;letter-spacing:.03em}.math-inline,.math-display{font-family:'Cambria Math','STIX Two Math',serif;color:#e9fdff}.math-display{display:block;overflow-x:auto;margin:.65em 0;padding:.45em;text-align:center;background:rgba(4,19,27,.62);border-left:2px solid #42ddff}.legacy-sup{font-family:'Cambria Math','STIX Two Math',serif;color:#e9fdff}
...
function mathElement(tag,value){const node=document.createElementNS(MATH_NS,tag);if(value!==undefined)node.textContent=value;return node}
function renderMath(source,display=false){let input=String(source||'').trim(),index=0;const commands={cdot:'·',times:'×',div:'÷',pm:'±',mp:'∓',le:'≤',leq:'≤',ge:'≥',geq:'≥',ne:'≠',neq:'≠',approx:'≈',infty:'∞',rightarrow:'→',leftarrow:'←',Rightarrow:'⇒',sum:'∑',prod:'∏',int:'∫',partial:'∂',nabla:'∇',alpha:'α',beta:'β',gamma:'γ',delta:'δ',theta:'θ',lambda:'λ',mu:'μ',pi:'π',rho:'ρ',sigma:'σ',phi:'φ',omega:'ω',Delta:'Δ',Sigma:'Σ',Omega:'Ω'};const functions=new Set(['sin','cos','tan','sec','csc','cot','log','ln','exp','lim','det']);
function skip(){while(index<input.length&&/\s/.test(input[index]))index++}
function one(){skip();if(index>=input.length)return null;if(input[index]==='{'){index++;const grouped=sequence('}');if(input[index]==='}')index++;return grouped}if(input[index]==='\\'){index++;const match=input.slice(index).match(/^[A-Za-z]+/),name=match?match[0]:input[index]||'';index+=name.length||1;if(name==='left'||name==='right')return one();if(name==='frac'){const fraction=mathElement('mfrac');fraction.append(one()||mathElement('mrow'));fraction.append(one()||mathElement('mrow'));return fraction}if(name==='sqrt'){const root=mathElement('msqrt');root.append(one()||mathElement('mrow'));return root}if(name==='text'){const content=one();const label=mathElement('mtext');label.textContent=content?content.textContent:'';return label}if(commands[name]!==undefined)return mathElement('mo',commands[name]);if(functions.has(name))return mathElement('mi',name);return mathElement('mi',name)}const char=input[index];if(/[0-9.,]/.test(char)){const match=input.slice(index).match(/^[0-9]+(?:[.,][0-9]+)?/)[0];index+=match.length;return mathElement('mn',match)}if(/[A-Za-z]/.test(char)){const match=input.slice(index).match(/^[A-Za-z]+/)[0];index+=match.length;return mathElement('mi',match)}index++;return mathElement(/[+\-=*/<>()[\]|]/.test(char)?'mo':'mi',char==='*'?'×':char)}
function sequence(end){const row=mathElement('mrow');while(index<input.length&&input[index]!==end){let base=one();if(!base)break;let sub=null,sup=null;skip();while(input[index]==='^'||input[index]==='_'){const kind=input[index++];const script=one()||mathElement('mrow');if(kind==='^')sup=script;else sub=script;skip()}if(sub&&sup){const wrapped=mathElement('msubsup');wrapped.append(base,sub,sup);base=wrapped}else if(sub){const wrapped=mathElement('msub');wrapped.append(base,sub);base=wrapped}else if(sup){const wrapped=mathElement('msup');wrapped.append(base,sup);base=wrapped}row.append(base)}return row}
const math=mathElement('math');math.setAttribute('display',display?'block':'inline');math.append(sequence());return math}
function appendTypography(host,value){const raw=String(value||''),pattern=/([A-Za-z0-9)\]])\^(?:\{([^}]+)\}|([-+]?\d+|[A-Za-z]))/g;let cursor=0,match;const appendText=part=>host.append(document.createTextNode(part.replace(/([A-Za-z0-9)\]])\s*\*\s*(?=[A-Za-z0-9(\[])/g,'$1 × ')));while((match=pattern.exec(raw))){appendText(raw.slice(cursor,match.index));appendText(match[1]);const sup=document.createElement('sup');sup.className='legacy-sup';sup.textContent=match[2]||match[3];host.append(sup);cursor=pattern.lastIndex}appendText(raw.slice(cursor))}
function appendInline(host,value){const raw=String(value||''),pattern=/(\\\[[\s\S]*?\\\]|\\\([\s\S]*?\\\)|\$\$[\s\S]*?\$\$|\$[^$\n]+\$|\*\*[^*\n]+\*\*|`[^`\n]+`|\*[^*\n]+\*)/g;let cursor=0,match;while((match=pattern.exec(raw))){appendTypography(host,raw.slice(cursor,match.index));const token=match[0];if(token.startsWith('\\[')||token.startsWith('$$')){const formula=token.slice(2,-2);const wrap=document.createElement('span');wrap.className='math-display';wrap.append(renderMath(formula,true));host.append(wrap)}else if(token.startsWith('\\(')||token.startsWith('$')){const formula=token.startsWith('$')?token.slice(1,-1):token.slice(2,-2);const wrap=document.createElement('span');wrap.className='math-inline';wrap.append(renderMath(formula,false));host.append(wrap)}else if(token.startsWith('**')){const strong=document.createElement('strong');appendInline(strong,token.slice(2,-2));host.append(strong)}else if(token.startsWith('`')){const code=document.createElement('code');code.className='md-inline-code';code.textContent=token.slice(1,-1);host.append(code)}else{const emphasis=document.createElement('em');appendInline(emphasis,token.slice(1,-1));host.append(emphasis)}cursor=pattern.lastIndex}appendTypography(host,raw.slice(cursor))}
function renderMarkdown(host,value){host.replaceChildren();const raw=String(value||'').replace(/\r\n?/g,'\n');if(!raw.trim()){host.textContent='—';return}const lines=raw.split('\n');let list=null,listType='',code=null;for(const line of lines){if(code){if(line.trim()==='```'){host.append(code);code=null}else code.textContent+=(code.textContent?'\n':'')+line;continue}if(line.trim().startsWith('```')){list=null;code=document.createElement('pre');code.className='md-code';continue}if(!line.trim()){list=null;continue}const heading=line.match(/^(#{1,4})\s+(.+)$/),bullet=line.match(/^\s*[-+]\s+(.+)$/),ordered=line.match(/^\s*\d+[.)]\s+(.+)$/),quote=line.match(/^>\s?(.*)$/);if(heading){list=null;const h=document.createElement('h'+Math.min(6,heading[1].length+2));h.className='md-heading';appendInline(h,heading[2]);host.append(h);continue}if(bullet||ordered){const kind=bullet?'ul':'ol';if(!list||listType!==kind){list=document.createElement(kind);list.className='md-list';listType=kind;host.append(list)}const li=document.createElement('li');appendInline(li,(bullet||ordered)[1]);list.append(li);continue}list=null;const block=document.createElement(quote?'blockquote':'div');block.className=quote?'md-quote':'md-p';appendInline(block,quote?quote[1]:line);host.append(block)}if(code)host.append(code)}
...
function renderArtifact(a){a=a||{};text('title',a.title||'STUDY MODULE');text('subject',a.subject||'READY');renderMarkdown(by('problem'),a.problem||'No problem supplied.');renderMarkdown(by('result'),a.result||'No result supplied.');const steps=by('steps');steps.replaceChildren();(a.steps||[]).forEach((s,i)=>{const d=document.createElement('div');d.className='step';const n=document.createElement('span');n.className='num';n.textContent=String(i+1).padStart(2,'0');const t=document.createElement('span');renderMarkdown(t,s);d.append(n,t);steps.appendChild(d)});const notes=by('notes');notes.replaceChildren();(a.notes||[]).forEach(n=>{const d=document.createElement('div');d.className='note';renderMarkdown(d,n);notes.appendChild(d)});const sources=by('sources');sources.replaceChildren();(a.sources||[]).forEach(s=>{const a=document.createElement('a');a.className='source';a.href=s.url;a.textContent=s.name;a.target='_blank';sources.appendChild(a)});const v=a.visualization||{};if(v.type==='plot2d'||v.type==='plot3d')renderPlot(v);else if(v.type==='free_body')renderForces(v);else if(v.type==='molecule')renderMolecule(v);else if(v.type==='anatomy')renderAnatomy(v);else if(v.type==='geogebra')renderGeoGebra(v);else by('visual').innerHTML='<div class="empty">SOLUTION DATA READY<br>NO VISUAL ARTIFACT REQUIRED</div>'}

...
def _derive_keys(session_key: str) -> tuple[bytes, bytes]:
    """Derive independent AES and HMAC keys from the one-time session key."""
    material = hashlib.sha512(session_key.encode('utf-8') + _AES_SALT).digest()
    return material[:32], material[32:]


def _decrypt_cbc(aes_key: bytes, mac_key: bytes, enc_b64: str) -> str:
    """Verify HMAC, then decrypt base64(IV ‖ ciphertext ‖ HMAC)."""
    from cryptography.hazmat.primitives.ciphers import Cipher, algorithms, modes
    from cryptography.hazmat.primitives import padding as sym_pad
    raw      = base64.b64decode(enc_b64)
    if len(raw) < 64:
        raise ValueError("Encrypted payload is too short")
    iv, ct, supplied_mac = raw[:16], raw[16:-32], raw[-32:]
...
class DashboardServer:
    ...
    def __init__(self):
        ...
        self._crypto_cache: dict[str, tuple[bytes, bytes]] = {}
    ...
    def new_key(self, expiry_secs: int = 600) -> str:
        ...
        key = ''.join(secrets.choice(_KEY_CHARS) for _ in range(SESSION_KEY_LENGTH))
    ...
    def _issue_token(self, session_key: str) -> str:
        ...
        self._token_expiry[token] = time.time() + AUTH_TOKEN_TTL_SECS
        return token

    def _token_valid(self, token: str) -> bool:
        if not token or token not in self._tokens:
            return False
        if self._token_expiry.get(token, 0) > time.time():
            return True
    ...
    def _login_allowed(self, client_ip: str) -> bool:
        cutoff = time.time() - LOGIN_WINDOW_SECS
        recent = [t for t in self._login_attempts.get(client_ip, []) if t > cutoff]
        self._login_attempts[client_ip] = recent
        return len(recent) < LOGIN_MAX_ATTEMPTS

    def _record_login_failure(self, client_ip: str) -> None:
        self._login_attempts.setdefault(client_ip, []).append(time.time())
    ...
    def get_manual_url(self) -> str:
        """URL for manual browser entry on the explicitly enabled dashboard port."""
        return f"{self._ip}:{PORT}"

    def _crypto_keys(self, session_key: str) -> tuple[bytes, bytes]:
        if session_key not in self._crypto_cache:
            self._crypto_cache[session_key] = _derive_keys(session_key)
        return self._crypto_cache[session_key]
    ...
    def _decrypt(self, token: str, enc_b64: str) -> str | None:
        ...
        if not sk:
            return None
        try:
            aes_key, mac_key = self._crypto_keys(sk)
            return _decrypt_cbc(aes_key, mac_key, enc_b64)
        except Exception:
            return None
...
<p>Connecting to JARVIS…</p>
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Firewall and network-profile changes are never made automatically.")

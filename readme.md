...
### Startup modes and local wake word

Direct mode does not require Vosk or a wake-word model and remains the safe default:

```bash
python jarvis_launcher.py --mode direct
```

Wake-word mode listens locally with Vosk while JARVIS is asleep. It releases the microphone before starting Gemini Live and resumes listening only after the main session exits; wake-word audio is not stored or sent to Gemini:

```bash
python jarvis_launcher.py --mode wake
```

Install a Vosk model under `models/` (the default is `models/vosk-model-small-en-us-0.15`). Configure phrases, model and RMS threshold with:
...
The generated `config/wake_word.json` is local and ignored by Git. Use `--disable` or `--enable` with `--configure` to turn listening off or on. In JARVIS settings, recreate the desktop shortcuts to obtain **J.A.R.V.I.S directo** and **J.A.R.V.I.S por voz**. If wake mode reports a missing model or microphone error, use direct mode and verify the configured model path and Windows microphone permissions.
...
├── jarvis_launcher.py       # Official direct/wake-word entry point
├── wake_word.py             # Local Vosk wake-word lifecycle

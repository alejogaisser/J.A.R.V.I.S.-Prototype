...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        # Backslash local: solo funciona cuando la ventana de Jarvis tiene el foco.
        ...
        try:
            ...
            if _os == "Windows":
                ...
                self._create_lnk_windows(
                    str(desktop / "J.A.R.V.I.S directo.lnk"), target,
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self._mute_btn = QPushButton("🎛  MODE: TOGGLE TO SPEAK · \\ OFF")
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addWidget(_fl("[\\] Toggle Speak  ·  [F9] Hardware Mute  ·  [F11] Fullscreen"))
        lay.addStretch()
    ...
    def _toggle_talk(self):
        """\\: abre/cierra el micrófono solamente en modo Toggle to Speak."""
    ...
    def _toggle_listen_mode(self):
        """Botón: alterna entre Toggle to Speak y Always Listening."""
        if self._listen_mode == "toggle":
    ...
    def _toggle_mute(self):
        """Compatibilidad interna. El mute físico se controla con F9 en Windows."""
        self._talk_enabled = not self._talk_enabled
    ...
    def _style_mute_btn(self):
        if self._listen_mode == "always":
            self._mute_btn.setText("🎙  MODE: ALWAYS LISTENING")
            ...
            self._mute_btn.setText("🎛  TOGGLE TO SPEAK · \\ ON")
            self._mute_btn.setStyleSheet(f"""
                QPushButton {{
                    background: #00140a; color: {C.GREEN};
                    border: 1px solid {C.GREEN}; border-radius: 3px;
            ...
            self._mute_btn.setText("🎛  TOGGLE TO SPEAK · \\ OFF")
            self._mute_btn.setStyleSheet(f"""
                QPushButton {{
...
class JarvisUI:
    def __init__(self, face_path: str, size=None):
        ...
        # Elimina cualquier ventana Qt fantasma creada durante el arranque.
    ...
    def _bring_main_window_to_front(self):
        ...
        try:
            ...
            print(f"[UI] No se pudo traer JARVIS al frente: {exc}")

    def _close_ghost_windows(self):
        for widget in QApplication.topLevelWidgets():
            if widget is self._win:
                continue

            if widget.isVisible():
                print(
                    f"[UI] Cerrando ventana extra: "
                    f"{type(widget).__name__} | "
                    f"título={widget.windowTitle()!r}"
                )
                widget.close()
                widget.deleteLater()
